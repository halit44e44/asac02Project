...
var run = function() {
    ...
    let apiUrl = api_url+'v2/get/cargo';
    let url = base_url+'cargo';

    var datatable = $('#data_cargo').KTDatatable({
        data: {
            type: 'remote',
        ...
        columns: [
            {
                ...
                template: function(data) {
                    ...
                    if (data.image !== null) {
                        image = site_url+'media/cargo/'+data.image;
                ...
                template: function(row) {
                    ...
                    return '<a data-id="'+row.id+'" data-table="cargo" data-tables="#data_cargo" onclick="status(this)"  <span class="btn btn-sm font-weight-bold label-lg ' + status[row.status].class + ' label-inline">' + status[row.status].title + '</span> </a>';
                },
            },{
                field: 'Actions',
                title: 'İşlemler',
                ...
                template: function(row) {
                    ...
                    return '<a class="btn btn-sm btn-clean btn-icon" data-table="cargo" data-id="'+row.id+'" onclick="editItem(this)"  title="Düzenle">\
                            <i class="fa fa-paint-brush text-warning"></i>\
                        </a>\
                        <a href="javascript:;" class="btn btn-sm btn-clean btn-icon" data-id="'+row.id+'" data-table="cargo" data-datatable="'+datatable+'"  onclick="removeItem(this)" title="Kaldır">\
    ...
    $('#status_cargo').on('change', function() {
        datatable.search($(this).val().toLowerCase(), 'status');
    });

    $('#status_cargo').selectpicker();
