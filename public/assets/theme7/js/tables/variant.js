...
var run = function() {
    ...
    let apiUrl = api_url+'v2/get/variant';
    let url = base_url+'variant';

    if (top_id > 0) {
        apiUrl = api_url+'v2/get/variant/'+top_id;
        url = base_url+'variant';
        $('.list-title span').addClass('dn');
        $('ul.breadcrumb').removeClass('dn');
    }

    var datatable = $('#data_variant').KTDatatable({
        data: {
            type: 'remote',
            source: {
                read: {
        ...
        columns: [
            {
                ...
                template: function(data) {
                    ...
                    if (data.image !== null) {
                        image = site_url+'media/variant/'+data.image;
                    }
                ...
                template: function(row) {
                    ...
                    return '<a data-id="'+row.id+'"  data-table="variant" data-tables="#data_variant" onclick="status(this)"  <span class="btn btn-sm font-weight-bold label-lg ' + status[row.status].class + ' label-inline">' + status[row.status].title + '</span> </a>';
                },
            },{
                field: 'Actions',
                title: 'İşlemler',
                ...
                autoHide: false,
                template: function(row) {
                    return '<a class="btn btn-sm btn-clean btn-icon" data-table="variant" data-id="'+row.id+'" onclick="editItem(this)"  title="Düzenle">\
                            <i class="fa fa-paint-brush text-warning"></i>\
                        </a>\
                        <a href="javascript:;" class="btn btn-sm btn-clean btn-icon" data-id="'+row.id+'" data-table="variant" data-datatable="'+datatable+'"  onclick="removeItem(this)" title="Kaldır">\
    ...
    $('#status_variant, #user_groups_cat').selectpicker();
