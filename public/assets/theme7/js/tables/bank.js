...
var run = function() {
    ...
    let apiUrl = api_url+'v2/get/bank';
    let url = base_url+'brand';

    var datatable = $('#data_bank').KTDatatable({
        data: {
            type: 'remote',
            source: {
                read: {
        ...
        columns: [
            {
                ...
                template: function(row) {
                    ...
                    return ' <a data-id="'+row.id+'" data-table="bank" data-tables="#data_bank" onclick="status(this)" <span class="btn btn-sm font-weight-bold label-lg ' + status[row.status].class + ' label-inline">' + status[row.status].title + '</span> </a>';
                },
            },
            {
                field: 'Actions',
                ...
                template: function(row) {
                    return '\
                        <a  class="btn btn-sm btn-clean btn-icon" data-table="bank" data-id="'+row.id+'" onclick="editItem(this)"  title="Düzenle">\
                            <i class="fa fa-paint-brush text-warning"></i>\
                        </a>\
                          <a href="javascript:;" class="btn btn-sm btn-clean btn-icon" data-table="bank" data-id="'+row.id+'" onclick="removeItem(this)" title="Kaldır">\
