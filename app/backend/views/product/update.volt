...
<div class="alert alert-secondary rounded-0 product-alert dn" role="alert"></div>
                        ...
                        <div class="w100 flex-shrink-0 bg-light p-5 product-preview">
                            <div id="previewImage">
                                {% if preview_image is not null %}
                                    <img class="fix-sidebar-image w100" src="{{ url('media/product/' ~ preview_image) }}" />
                                {% elseif products.purchase_rate is 2 %}
                                    <img class="fix-sidebar-image w100" src="{{ url('media/product/resimyok.jpg') }}" />
                                {% endif %}
                            </div>
                            <div class="mt-5">
                                <h4 class="font-size-h5">
                                    <a href="javascript:;" class="text-dark-75 font-weight-bold text-hover-primary" id="preview_name">{{ products.name }}</a>
                                </h4>

                                {% set purchase_currency = 'TL' %}
                                {% if products.purchase_rate is "2" %}
                                    {% set purchase_currency = 'USD' %}
                                {% elseif products.purchase_rate is "3" %}
                                    {% set purchase_currency = 'EURO' %}
                                {% endif %}

                                <div class="text-muted" id="preview_price"><span class="text-dark-75 font-weight-bolder">Satış Fiyatı: </span> <span class="price">{{ products.purchase_price }}</span><span class="currency"> {{ purchase_currency }}</span></div>

                                {% set dahil = 'KDV Dahil' %}
                                {% if products.vat_definition is '2' %}
                                    {% set dahil = 'KDV Hariç' %}
                                {% endif %}

                                <div class="text-muted" id="preview_vat"><span class="text-dark-75 font-weight-bolder">{{ dahil}} / {{ products.vat_rate }}%</span></div>

                                {% set discount = '' %}
                                {% if products.discount_type is "1" %}
                                    {% set discount = purchase_currency %}
                                {% elseif products.discount_type is "2" %}
                                    {% set discount = '%' %}
                                {% endif %}

                                {% if products.discount_rate is not '0' %}
                                    <div class="text-muted" id="preview_discount"><span class="text-dark-75 font-weight-bolder">İndirim:</span> <span class="discount">{{ products.discount_rate }} {{ discount }}</span></div>
                                {% endif %}

                                {% if products.transfer_discount is not 0 %}
                                <div class="text-muted" id="preview_transfer_discount"><span class="text-dark-75 font-weight-bolder">Havale indirimi:</span> <span class="discount">{{ products.transfer_discount }} {{ discount }}</span></div>
                                {% endif %}

                                {% if products.shipping_fee is not '' %}
                                <div class="text-muted" id="preview_shipping_fee"><span class="text-dark-75 font-weight-bolder">Kargo ücreti:</span> <span class="fee">{{ products.shipping_fee }} {{ purchase_currency }}</span></div>
                                {% endif %}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="form-group mb-8">
                            ...
                            <select class="form-control form-control-solid form-control-lg picker" data-live-search="true" multiple="multiple" name="top_id[]">
                                {% if pro is defined %}
                                    {% for pro in pro %}
                                        <?php $tops = explode(",", $products->top_id); ?>
                                        <option value="{{ pro.id }}" {% if pro.id in tops %}selected{% endif %}>{{ pro.name }}</option>
                                    {% endfor %}
                                {% endif %}
                            </select>
                        </div>
                        ...
                        <div class="catArea">
                            <div class="form-group mb-8">
                                ...
                                <input class="form-control form-control-solid form-control-lg" type="text" id="catPicker" placeholder="Kategori seç"/>
                                ...
                                {% if features is defined %}
                                    {% for feature in features %}
                                        <optgroup label="{{ feature.name }}">
                                            <?php
                                                $subvars = \Yabasi\Feature::find('top_id='.$feature->id);
                                                if ($subvars) {
                                                    $featureSelectList = explode(",", $products->feature_id);
                                                    foreach ($subvars as $sub) {
                                                        if(in_array($sub->id, $featureSelectList)){
                                            ?>
                            ...
                            <select class="form-control form-control-solid form-control-lg picker variant_id" data-live-search="true" multiple="multiple" name="variant_id[]">
                                {% if variant is defined %}
                                    ...
                                    <optgroup label="{{ variant.name }}" data-id="{{ variant.id}} ">
                                        <?php
                                            $subvars = \Yabasi\Variant::find('top_id='.$variant->id);
                                            if ($subvars) {
                                                foreach ($subvars as $sub) {
                                                    if(in_array($sub->id, $arrvariant)){
                                        ?>
                                            <option value="{{ sub.id }}" selected="selected" data-id="{{ sub.name }}" data-top_id="{{ sub.top_id }}">{{ sub.name }}</option>
                                        <?php }else{ ?>
                            ...
                            <select class="form-control form-control-solid form-control-lg picker" data-live-search="true" multiple="multiple" name="gift_id[]">
                                {% if products is defined %}
                                    {% for pro in products_list %}
                                        <?php $gifts = explode(",", $products->gift); ?>
                                        <option value="{{ pro.id }}" {% if pro.id in gifts %}selected{% endif %}>{{ pro.name }}</option>
                                    {% endfor %}
                                {% endif %}
                            </select>
                        </div>
                        ...
                        <div class="form-group mb-8">
                            ...
                            <select class="form-control form-control-solid form-control-lg picker" data-live-search="true" multiple="multiple" name="recommended_products[]">
                                {% if products is defined %}
                                    {% for pro in products_list %}
                                        <?php $recom = explode(",", $products->recommended_products); ?>
                                        <option value="{{ pro.id }}" {% if pro.id in recom %}selected{% endif %}>{{ pro.name }}</option>
                                    {% endfor %}
                                {% endif %}
                            </select>
                        </div>
                        <button type="submit" class="btn btn-primary font-weight-bolder mr-2 mb-10 px-8 col-md-12">Kaydet</button>
                    </div>
                            ...
                            <div class="card card-custom card-shadowless">
                                <div class="card-body p-0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control form-control-solid" id="name" name="name" value="{% if products.name is defined %}{{ products.name }}{% endif %}">
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control form-control-solid" id="stock_code" name="stock_code" value="{% if products.stock_code is defined %}{{ products.stock_code }}{% endif %}">
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control form-control-solid" id="barcode_code" name="barcode_code" value="{% if products is defined %}{{ products.barcode }}{% endif %}">
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="number" min="0" class="form-control form-control-solid" id="stock_quantity" name="stock_quantity" value="{% if products is defined %}{{ products.unit }}{% endif %}">
                                            <div class="input-group-append">
                                                <select class="form-control form-control-solid" id="stock_type" name="stock_type">
                                                    <option value="">Seçim yapınız</option>
                                                    {% set stock_type_list = ['Adet', 'Cm', 'Düzine', 'Gram', 'Kg', 'Kişi', 'Paket', 'Metre', 'M2', 'Çift'] %}
                                                
                                                    {% for value in stock_type_list %}
                                                        {% if products.unit_type == loop.index %}
                                                            <option value="{{ loop.index }}" selected="selected">{{ value }}</option>
                                                        {% else %}
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="number" min="0" class="form-control form-control-solid" id="cargo_weight" name="cargo_weight" value="{% if products is defined %}{{ products.cargo_weight }}{% endif %}">
                                            <div class="input-group-append">
                            ...
                            <div class="card card-custom card-shadowless">
                                <div class="card-body p-0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control form-control-solid" id="purchase_price" name="purchase_price" value="{% if products is defined %}{{ products.purchase_price }}{% endif %}"  >
                                            <div class="input-group-append">
                                                <select class="form-control form-control-solid" id="purchase_price_exchange_rate" name="purchase_price_exchange_rate" >                                                    
                                                    {% for value in currency_list %}
                                                        {% if products.purchase_rate == loop.index %}
                                                            <option value="{{ loop.index }}" selected="selected">{{ value }}</option>
                                                        {% else %}
                                                            <option value="{{ loop.index }}">{{ value }}</option>
                                                        {% endif %}
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control form-control-solid" id="market_price" name="market_price" value="{% if products is defined %}{{ products.market_price }}{% endif %}" >
                                            <div class="input-group-append">
                                                <select class="form-control form-control-solid" id="market_price_exchange_rate" name="market_price_exchange_rate" >                                                    
                                                    {% for value in currency_list %}
                                                        {% if products.market_rate == loop.index %}
                                                            <option value="{{ loop.index }}" selected="selected">{{ value }}</option>
                                                        {% else %}
                                                            <option value="{{ loop.index }}">{{ value }}</option>
                                                        {% endif %}
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control form-control-solid" id="sale_price" name="sale_price" value="{% if products is defined %}{{ products.sale_price }}{% endif %}" >
                                            <div class="input-group-append">
                                                <select class="form-control form-control-solid" id="sale_price_exchange_rate" name="sale_price_exchange_rate" >                                                        
                                                    {% for value in currency_list %}
                                                        {% if products.sale_rate == loop.index %}
                                                            <option value="{{ loop.index }}" selected="selected">{{ value }}</option>
                                                        {% else %}
                                                            <option value="{{ loop.index }}">{{ value }}</option>
                                                        {% endif %}
                                    ...
                                    <div class="form-group row">
                                        <div class="col-lg-4">
                                            ...
                                            <select class="form-control form-control-solid" id="vat_definition" name="vat_definition" >
                                                {% if products.vat_definition == "1" %}
                                                    <option value="1" selected>KDV Dahil</option>
                                                    <option value="2">KDV Hariç</option>
                                                {% else %}
                                                    <option value="1">KDV Dahil</option>
                                                    <option value="2" selected>KDV Hariç</option>
                                                {% endif %}
                                            </select>
                                        </div>
                                        <div class="col-lg-4">
                                            <label for="vat_rate">KDV Oranı</label>
                                            <input type="number" min="0" max="100" class="form-control form-control-solid" id="vat_rate" name="vat_rate" value="{% if products.vat_rate is defined %}{{ products.vat_rate }}{% endif %}">
                                        </div>
                                        <div class="col-lg-4">
                                            <label for="shipping_fee">Kargo Ücreti</label>
                                            <input type="text" class="form-control form-control-solid" id="shipping_fee" name="shipping_fee" value="{% if products.shipping_fee is defined %}{{products.shipping_fee}}{% endif %}">
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-lg-4">
                                            ...
                                            <select class="form-control form-control-solid" id="discount_type" name="discount_type" >
                                                ...
                                                {% if products.discount_type == "1" %}
                                                    <option value="price" selected>Fiyat</option>
                                                    <option value="percentage">Yüzdesel</option>
                                                {% else %}
                                                    <option value="price">Fiyat</option>
                                                    <option value="percentage" selected>Yüzdesel</option>
                                                {% endif %}
                                            </select>
                                        </div>
                                        <div class="col-lg-4">
                                            <label for="discount_rate">İndirim Oranı</label>
                                            <input type="number" class="form-control form-control-solid" min="0" id="discount_rate" name="discount_rate" value="{% if products.discount_rate is defined %}{{products.discount_rate}}{% endif %}">
                                        </div>
                                        <div class="col-lg-4">
                                            <label for="transfer_discount">Havale İndirimi</label>
                                            <input type="number" class="form-control form-control-solid" min="0" id="transfer_discount" name="transfer_discount" value="{% if products.transfer_discount is defined %}{{products.transfer_discount}}{% endif %}">
                                        </div>
                                    </div>
                            ...
                            <div class="card card-custom card-shadowless">
                                <div class="card-body p-0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control form-control-solid" rows="4" name="short_content" id="short_content">{% if products.short_content is defined %}{{products.short_content}}{% endif %}</textarea>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control form-control-solid" name="content" id="content">{{products.content}}</textarea>
                                    </div>
                            ...
                            <div class="card card-custom card-shadowless">
                                <div class="card-body p-0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="seo_title" id="seotitle" type="text" class="form-control form-control-solid" value="{% if products.seo_title is defined %}{{products.seo_title}}{% endif %}"/>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input name="slugurl" type="text" class="form-control form-control-solid slugurl" value="{% if products.sef is defined %}{{products.sef}}{% endif %}"/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="keyword" id="etiket" class="form-control form-control-solid tagify" value='{% if products.keyword is defined %}{{products.keyword}}{% endif %}'/>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea type="text" rows="2" class="form-control form-control-solid" id="description" maxlength="150" name="description">{{products.description | trim}}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card card-custom card-shadowless">
                                <div class="card-body p-0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="search_keywords" id="search_keywords" class="form-control form-control-solid tagify" value="{% if products.search_keywords is defined %}{{products.search_keywords}}{% endif %}"  />
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="page_keywords" id="page_keywords" class="form-control form-control-solid tagify" value="{% if taglist is defined %}{{taglist}}{% endif %}"  />
                                    </div>
...
<div class="d-flex flex-column-fluid {% if variants is not defined %}dn{% endif %}" id="variant_options">
        ...
        <div class="col-lg-9 col-sm-12 float-right padding-none-m padding-right-none">
            <div class="mb-10 dns">
                <div class="card mb-10 dns">
                    <div class="spinner spinner-primary dn spinner-lg mr-15"></div>
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-7">
                        ...
                        <form id="variantOptionsForm">
                            <input type="hidden" name="id" value="{{ products.id }}" class="lastid" />
                            <table class="table table-borderless">
                                ...
                                <tbody>
                                {% if variants is defined %}{{ variants }}{% endif %}
                                </tbody>
                            </table>
...
<div class="d-flex flex-column-fluid photoUploadConteiner">
 <div class="container padding-right-none">
    <div class="col-lg-9 col-sm-12 float-right padding-none-m padding-right-none">
        ...
        <div class="card mb-10 dns">
            <div class="card-body">
                ...
                <form class="dropzone dropzone-default dropzone-primary photoupload">
                    <input type="hidden" class="lastid" name="id" value="{{ products.id }}" />
            ...
            <div class="mb-10 dns">
                <div id="uploadedphotos">
                    {% if images is defined %}
                              ...
                              <div class="d-flex">
                                <div class="flex-shrink-0 mr-7">
                                  <div class="symbol symbol-50 symbol-lg-120">
                                    <img src="media/{{ image.meta_key }}/{{ image.meta_value }}" style="object-fit: cover;" />
                                  </div>
                                </div>
                                <div class="flex-grow-1">
                                  <form id="form_{{ image.id }}">
                                    <input type="hidden" class="lastid" name="id" value="{{ image.id }}" />
                                    <input type="hidden" name="extension" value="<?php echo explode(".", $image->getMetaValue())[1]; ?>" />
                                    <div class="d-flex align-items-center flex-wrap justify-content-between row">
                                      <div class="col-md-10">
                                        <div class="form-group">
                                          <label>Dosya ismi</label>
                                          <input type="text" class="form-control" name="name" value="<?php echo explode(".", $image->getMetaValue())[0]; ?>" autocomplete="off"/>
                                        </div>
                                      </div>
                                      <div class="col-md-2">
                                        <div class="form-group">
                                          <label>Sıra</label>
                                          <input type="number" class="form-control" name="row" value="{{ image.row }}" min="1" autocomplete="off"/>
                                        </div>
                                      </div>
                                      <div class="col-md-6">
                                        <label class="checkbox">
                                            {% if image.showcase == 1 %}
                                                <input type="checkbox" name="showcase" value="1" data-image="media/{{ image.meta_key }}/{{ image.meta_value }}" checked="true">
                                            {% else %}
                                                <input type="checkbox" name="showcase" value="0" data-image="media/{{ image.meta_key }}/{{ image.meta_value }}">
                                            {% endif %}
                                      ...
                                      <div class="col-md-6 text-right">
                                        <button type="button" onclick="removeImage(this)" data-id="{{ image.id }}" class="btn btn-light-danger mr-2">Kaldır</button>
                        ...
                        {% endfor %}
                        <script>
                            $(document).ready(function () { changePreviewImage(); });
                        </script>                    
                    {% endif %}
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        ...
        const categoryPicker = $('#catPicker').comboTree({
			...
			cascadeSelect: true,
			collapse: false,
            selected: [{% if products.cats_id is defined %}{{products.cats_id}}{% endif %}]
		});
        ...
        $('#proForm').validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: 'post',
                    url: '{{ url("backend/product/update/" ~ products.id) }}',
                    data: data,
                    success: function (response) {
                        ...
                        if (obj.status === 'ok') {
                            ...
                            $('.lastid').val(obj.id);
                            $('.product-alert').removeClass('dn').removeClass('alert-danger').addClass('alert-secondary').html('İşlem başarılı bir şekilde tamamlandı!');
                            $('html, body').animate({ scrollTop: $("#variant_options").offset().top }, 2000);

                        } else {
                            $('.product-alert').removeClass('dn').removeClass('alert-secondary').addClass('alert-danger').html('Bir sorun oluştu. Lütfen daha sonra tekrar deneyiniz!');
                            goTop();
                        }
                    }
                });
    ...
    function variant(dizi,id,dizi2,id2,dizi3,id3,dizi4,id4) {
        ...
        console.log(dizi,id,dizi2,id2,dizi3,id3,dizi4,id4);
    ...
    if ($('form.photoupload').length) {
        const photoUpload = new Dropzone('form.photoupload', {
            ...
            thumbnail: function(file, dataUrl) {
                $("#previewImage").html('<img src="'+dataUrl+'" class="w100">');
                // if($("#previewImage").attr("src") == "/media/product/resimyok.jpg"){}
            },
    ...
    function showPreview() {
        ...
        $('#name').on('input', function () {
            ...
            if (val.length > 0) {
                ...
                product_preview.removeClass('dn');
            } else {
                preview_name.html('');
                product_preview.addClass('dn');
            }
        });
    ...
    function removeVar(el) {
        ...
        $('#row_'+id).addClass('variantbar');
        setTimeout(function () {
            $.ajax({
                type: 'post',
                data: 'id='+id,
                url: '{{ url('backend/product/deletevar') }}',
                success: function(response) {
                    if (response === 'ok') {
                        $('#row_'+id).html('');
                    }
    ...
    function loading(){
        $('#variant_options .card-body').css('opacity', '0.3');
        $('#variant_options .spinner').removeClass('dn');
        $('#variant_options .spinner').css('top', '200px');
        $('#variant_options .spinner').css('left','50%');

        setTimeout(function(){
            $('#variant_options .card-body').css('opacity', 'unset');
            $('#variant_options .spinner').removeClass('db');
            $('#variant_options .spinner').css('top', 'unset');
            $('#variant_options .spinner').css('left','unset');
            $('#variant_options .spinner').addClass('dn');
