...
<form  id="variantForm" method="post">
    <input type="hidden" name="id" value="{% if variant is defined %}{{ variant.id }}{% endif %}">
        ...
        <div class="card-body">
            ...
            <div class="form-group row">
                <div class="col-lg-6">
                    ...
                    <input type="text" id="name" name="name" value="{% if variant is defined %}{{ variant.name }}{% endif %}" class="form-control" placeholder="Varyasyon adını yazınız" />
                </div>
                <div class="col-lg-6">
...
<div class="card card-custom gutter-b photoCard" data-card="true">
    ...
    <div class="card-body">
        <form class="dropzone dropzone-default dropzone-primary photoupload">
            <input type="hidden" class="lastid" name="id" value="{% if variant is defined %}{{ variant.id }}{% endif %}" />
                ...
                <div class="d-flex">
                    <div class="flex-shrink-0 mr-7">
                        <div class="symbol symbol-50 symbol-lg-120">
                            <img src="{{ url('media/variant/' ~ image.meta_value) }}" style="object-fit: cover;" />
                        </div>
                    </div>
                    <div class="flex-grow-1">
                        <form id="form_{{ image.id }}">
                            <input type="hidden" class="lastid" name="id" value="{{ image.id }}" />
                            <input type="hidden" name="table" value="variant" />
                            ...
                            <div class="d-flex align-items-center flex-wrap justify-content-between row">
                                ...
                                <div class="col-md-6 text-right">
                                    <button type="button" onclick="removeImage(this)" data-id="{{ image.id }}" data-table="variant" class="btn btn-light-danger mr-2">Kaldır</button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        const categoryPicker = $('#catPicker').comboTree({
            ...
            cascadeSelect: true,
            collapse: false,
            selected: [{% if variant.top_id is defined %}{{variant.top_id}}{% endif %}]
        });
        ...
        $('#variantForm').validate({
            ...
            submitHandler: function () {
                ...
                $.ajax({
                    type: 'post',
                    url: '{{ url("backend/variant/update/") }}',
                    data: data,
